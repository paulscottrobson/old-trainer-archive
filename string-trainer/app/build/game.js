...
var MainState = (function (_super) {
    ...
    MainState.prototype.create = function () {
        ...
        this.manager = new DisplayManager(this.game, this.music, this.instrument, this.game.width, this.game.height / 2);
...
var Metronome = (function (_super) {
    ...
    function Metronome(game) {
        ...
        _this.metroImg.anchor.setTo(0.5, 0.91);
...
var MusicPlayer = (function (_super) {
    ...
    function MusicPlayer(game, music, instrument) {
        var _this = _super.call(this, game) || this;
        _this.music = music;
        ...
        _this.stringSound = [];
        for (var n = 0; n < _this.instrument.getVoices(); n++) {
            _this.stringSound[n] = _this.loadInstrument(_this.instrument.getTuning()[n]);
        }
        return _this;
    }
    ...
    MusicPlayer.prototype.update = function () {
        ...
        if (this.isInstrumentOn && !this.isPaused) {
            ...
            if (this.nextNote < bar.count && this.mbPosition >= bar.strums[this.nextNote].time) {
                for (var v = 0; v < this.instrument.getVoices(); v++) {
                    var note = bar.strums[this.nextNote].fretting[v];
        ...
        if (!this.isPaused) {
            ...
            if (this.mbPosition >= 1000) {
                this.mbPosition = 0;
                this.barPosition++;
                if (this.barPosition >= this.music.getBarCount()) {
                    this.barPosition = 0;
                }
                this.moveTo(this.barPosition, this.mbPosition);
            }
        }
        if (!this.isPaused) {
    ...
    MusicPlayer.prototype.moveTo = function (bar, mbPosition) {
        ...
        this.nextNote = 0;
        for (var n = 0; n < this.music.getBar(bar).count; n++) {
            if (this.music.getBar(bar).strums[n].time < mbPosition) {
                this.nextNote = n + 1;
            }
        }
    };
...
var BaseInstrument = (function () {
    function BaseInstrument(musObj) {
        this.tuning = musObj.tuning.toUpperCase().split(",");
    }
    BaseInstrument.prototype.getTuning = function () {
        return this.tuning;
    };
    BaseInstrument.prototype.getVoices = function () {
        return this.tuning.length;
    };
    BaseInstrument.prototype.getVolume = function (str) {
        return 1;
    };
    BaseInstrument.prototype.getFretNameFromChromatic = function (fret) {
...
var MountainDulcimer = (function (_super) {
    ...
    MountainDulcimer.prototype.getFretNameFromChromatic = function (fret) {
        var n = MountainDulcimer.NAMES[fret % 12];
        var s = (Math.floor(n) + Math.floor(fret / 12) * 7).toString();
        if (n != Math.floor(n)) {
            s = s + "+";
        }
        return s;
    };
    MountainDulcimer.prototype.getType = function () {
        return InstrumentType.DULCIMER;
    };
    return MountainDulcimer;
}(BaseInstrument));
MountainDulcimer.NAMES = [
    0, 0.5, 1, 1.5, 2, 3, 3.5, 4, 4.5, 5, 6, 6.5
];
var InstrumentType;
(function (InstrumentType) {
    InstrumentType[InstrumentType["DULCIMER"] = 0] = "DULCIMER";
...
var Music = (function () {
    function Music(musObj, instrument) {
        this.musObj = musObj;
        this.bar = [];
        for (var _i = 0, _a = this.musObj.bars; _i < _a.length; _i++) {
            var barSrc = _a[_i];
            var b = new Bar(this.bar.length, instrument.getVoices(), this.getStandardBeats(), barSrc);
            this.bar.push(b);
        }
    }
...
var PreloadState = (function (_super) {
    ...
    PreloadState.prototype.preload = function () {
        ...
        var src = SeagullMerlinApplication.getURLName("music", "music.json");
        this.game.load.json("music", SeagullMerlinApplication.getURLName("music", src));
        this.game.load.onLoadComplete.add(function () { _this.game.state.start("Main", true, false, 1); }, this);
    };
    return PreloadState;
}(Phaser.State));
PreloadState.NOTE_COUNT = 46;
var DisplayManager = (function (_super) {
    __extends(DisplayManager, _super);
    function DisplayManager(game, music, instrument, width, height) {
        ...
        for (var n = 0; n < _this.music.getBarCount(); n++) {
            var rnd = new HorizontalScrollRenderer(game, music.getBar(n), music, instrument, _this.barWidth, _this.drawHeight);
            _this.add(rnd);
            rnd.visible = false;
            _this.renderer[n] = rnd;
        }
        for (var p = 1; p <= 4; p++) {
            _this.renderer[-p] = new HorizontalScrollRenderer(game, null, music, instrument, _this.barWidth, _this.drawHeight);
...
var HorizontalScrollRenderer = (function (_super) {
    ...
    function HorizontalScrollRenderer(game, bar, music, instrument, width, height) {
        var _this = _super.call(this, game) || this;
        _this.rWidth = width;
        ...
        _this.music = music;
        _this.instrument = instrument;
        _this.voices = instrument.getVoices();
    ...
    HorizontalScrollRenderer.prototype.renderBar = function () {
        ...
        var c = (this.voices == 3) ? 4 : this.voices;
        for (var s = 0; s < c; s++) {
    ...
    HorizontalScrollRenderer.prototype.drawNotesSeperate = function () {
        ...
        for (var _i = 0, _a = this.bar.strums; _i < _a.length; _i++) {
            ...
            for (var strn = 0; strn < this.voices; strn++) {
                if (strum.fretting[strn] != Strum.SILENT) {
                    ...
                    var s1 = this.instrument.getFretNameFromChromatic(n);
