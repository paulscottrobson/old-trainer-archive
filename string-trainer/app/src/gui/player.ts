...
class MusicPlayer extends Phaser.Group implements IPlayer {
    ...
    constructor(game:Phaser.Game,music:IMusic,instrument:IInstrument) {
        super(game);
        this.music = music;this.instrument = instrument;
        ...
        for (var n:number = 0;n < this.instrument.getVoices();n++) {
            this.stringSound[n] = this.loadInstrument(this.instrument.getTuning()[n]);
        }
    }
    ...
    update(): void {
        ...
        if (this.isInstrumentOn && !this.isPaused) {
            ...
            if (this.nextNote < bar.count && this.mbPosition >= bar.strums[this.nextNote].time) {
                //console.log("Play",this.barPosition,this.nextNote,bar.strums[this.nextNote].toString());
                // for each voice identify the note
                for (var v:number = 0;v < this.instrument.getVoices();v++) {
                    var note:number = bar.strums[this.nextNote].fretting[v];
    ...
    moveTo(bar: number, mbPosition: number): void {
        ...
        this.nextNote = 0;
        for (var n:number = 0;n < this.music.getBar(bar).count;n++) {
