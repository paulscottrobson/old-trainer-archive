...
type NoteEvent
	...
	chromatic as integer[3]																			// Chromatic fret position of note, 99 if not played.
	isDownStroke as integer 																		// Non zero if down/away stroke
endtype

type ChordEvent
	time as integer																					// Event time in millibars (1000 millibars in each bar)
	name$ as string																					// Name of new chord
	chromatic as integer[3]																			// Chromatic fre position of note, 99 if not played.
endtype
...
type Song
	...
	type$ as string																					// instrument type
	tuning$ as string 																				// instrument tuning
	stringBase as integer[1]																		// the note numbers for each unfretted string, lowest first (e.g. tuning)
	...
	toDiatonic# as float[1]																			// Convert chromatic to diatonic table.
...
function COMSetup()
	...
	ctl.barX = 140
	ctl.barWidth = 530
	ctl.fretY = 320																					// Fretboard position
	ctl.fretHeight = 330
	...
	SetWindowTitle("Dulcimer Trainer")																// Screen set up
