...
function MusicSelector_New(mse ref as MusicSelector,itemList$ as String,iWidth as integer,iHeight as integer,depth as integer,vCount as integer,vSpacing as integer,baseID as integer)
	...
	CreateText(baseID,"Build "+str(BUILD_NUMBER)+" ("+BUILD_DATE+") written by Paul Robson 2016")
	SetTextSize(baseID,ctl.screenWidth/40)
	SetTextPosition(baseID,ctl.screenWidth/2-GetTextTotalWidth(baseID)/2,ctl.screenHeight-GetTextTotalHeight(baseID))
