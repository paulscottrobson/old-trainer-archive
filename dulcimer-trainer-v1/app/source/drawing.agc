...
function DRAWFretboard()
	...
	for s = 1 to 4																						// Draw 4 strings (2 melody)
		...
		w = 100-(s-1)*12																				// Make strings smaller as we go
		SetSpriteSize(SPR_STRING+s,ctl.screenWidth,w*ctl.screenHeight/80/100)
		if s > 2 then offset = (s*2-7) * ctl.screenHeight/80 else offset = 0							// Offset strings 3 + 4 which are the same base position
		SetSpritePosition(SPR_STRING+s,0,__DRAWGetStringY(s)+offset)
		if s < 4
			CreateText(SPR_STRING+s,mid("DAD",s,1))
			SetTextSize(SPR_STRING+s,ctl.screenWidth/10)
			SetTextDepth(SPR_STRING+s,DEPTH_FRETBOARD+3)
			SetTextPosition(SPR_STRING+s,4,__DRAWGetStringY(s)-GetTextTotalHeight(SPR_STRING+s)/2)
		endif
	next s
...
function __DRAWDrawBar(song ref as Song,bar as integer)
	...
	for n = 1 to song.bars[bar].noteCount 			
		...
		if song.bars[bar].notes[n].name$ <> ""															// Chord ?
			id = baseID + n * 10 + 400
			s$ = song.bars[bar].notes[n].name$															// Create chord name
			s$ = Upper(left(s$,1))+lower(mid(s$,2,99))
			CreateText(id,s$)																			// Create and size text
			sz# = ctl.barWidth / 8
			SetTextSize(id,sz#)
			for s = 1 to song.strings																	// Create the notes that make up the chord
				s$ = "-"
				if song.bars[bar].notes[n].chromatic[s] <> 99
					n# = song.toDiatonic#[song.bars[bar].notes[n].chromatic[s]]
					s$ = str(floor(n#))
					if n# <> floor(n#) then s$ = s$ + "+"
				endif
				CreateText(id+s,s$)
				SetTextSize(id+s,sz#/3)
			next s
			for s = 0 to song.strings																	// Set depth and colour
				SetTextDepth(id+s,DEPTH_BAR)
				SetTextColor(id+s,0,0,0,255)
			next s
		endif
		...
		if song.bars[bar].isStrummed = 0													
			for s = 1 to song.strings																	// Note boxes
				...
				if song.bars[bar].notes[n].chromatic[s] <> 99
					...
					SetSpriteFlip(id,0,song.bars[bar].notes[n].isDownStroke = 0)
					...
					chromatic = song.bars[bar].notes[n].chromatic[s]									// Chromatic note
					diatonic# = song.toDiatonic#[chromatic]												// Diatonic note
					s$ = str(floor(diatonic#))															// As a string, integer part
					if diatonic# <> floor(diatonic#) then s$ = s$ + "+"									// Add + for .5 diatonic
					SetTextString(id,s$)	
					col = floor(diatonic#)
					if diatonic# <> floor(diatonic#) then col = col + 7																
			...
			if song.bars[bar].notes[n].isDownStroke = 0 												// Handle up/toward strum
...
function DRAWMoveBar(song ref as Song,bar as integer,x as integer)
	...
	for n = 1 to song.bars[bar].noteCount 												
		...
		if song.bars[bar].notes[n].name$ <> ""															// Chord ?
			id = baseID + n * 10 + 400			
			SetTextPosition(id,xc-GetTextTotalWidth(id)/2,ctl.fretY-GetTextTotalHeight(id))
			for s = 1 to song.strings
				x1 = GetTextX(id)+GetTextTotalWidth(id)+6
				y1 = GetTextTotalHeight(id)*((s-2) * 20 + 65) / 100 + GetTextY(id)
				SetTextPosition(id+s,x1-GetTextTotalWidth(id+s)/2,y1-GetTextTotalHeight(id+s))
			next s
		endif
		...
		if song.bars[bar].isStrummed = 0
			for s = 1 to song.strings																		// Note boxes
				...
				id = baseID + n * 10 + s
				if song.bars[bar].notes[n].chromatic[s] <> 99
					if song.bars[bar].notes[n].isDownStroke then p = 50 else p = 50
