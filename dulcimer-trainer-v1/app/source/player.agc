...
function PLAYSetup(song ref as Song)
	...
	for s = 1 to song.strings+1 																		// For each of 4 strings
		base = (s-1) * 24 + 1
		if s = song.strings+1 then baseNote = song.stringBase[s-1] else baseNote = song.stringBase[s]	// Figure out where we start
		for n = 0 to 23 																				// 20 chromatic notes
...
function PLAYUpdate(song ref as Song,position# as float,lastPosition# as float)
	...
	for n = 1 to song.bars[bar].noteCount																// Check notes in bar to see if one is due 
		...
		if time >= firstBeat and time < lastBeat and ctl.musicOn <> 0									// If in range and actually playing
			for s = 1 to song.strings																	// Work through strings
				chromatic = song.bars[bar].notes[n].chromatic[s]										// Get chromatic value
				if chromatic <> 99																		// If plucked/strummed.
					...
					if s = song.strings																	// Handle melody string
						__PLAYChannels[s+1].nextFire = GetMilliseconds()
						__PLAYChannels[s+1].nextNoteID = chromatic + __PLAYChannels[s+1].baseSoundID
					endif						
	...
	tMs = GetMilliseconds()	
	for s = 1 to song.strings+1																			// Check if each due
