...
class GameState extends Phaser.State {
    ...
    create() : void {
        ...
        if (dType == "horizontal") {
            this.manager = new HorizontalDisplayManager(this.game,this.music,
                                                        this.game.width,this.game.height*0.45);
        } else {
            if  (dType == "stab") {
                this.manager = new ScrollingTabDisplayManager(this.game,this.music,
                                                              this.game.width-40,this.game.height*0.8);
                this.manager.x = 20;                                                                                                                            
            } else {
                this.manager = new StaticTabDisplayManager(this.game,this.music,
                                                              this.game.width-40,this.game.height*0.8);                
                this.manager.x = 20;                                                                                                                            
            }
        }
        ...
        this.player.x = this.game.width-15-this.player.width;this.player.y = this.panel.y;
        ...
        this.player.onPlayUpdate.add((obj:any,bar:number,millibeats:number) => {
