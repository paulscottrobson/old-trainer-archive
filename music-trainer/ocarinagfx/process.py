...
class Processor:
	...
	def generate(self):
		for s in self.source:
			if s[0] == '@':												# @xxxx changes current base image
				...
				self.create(self.currentImage,s[1],name,s[0],self.nextNote)			# generate image
		...
		while self.nextNote < 26:										# lots of blank ones to 25
			self.createEmpty(self.nextNote)
			self.nextNote += 1
	#
	#	Unknown.
	#
	def createEmpty(self,n):
		...
		name="{0}/{1}.png".format(self.directory,n)
		image.save(name);

	#
	#	Create a single OCPIX image
	#
	def create(self,baseImage,holes,targetImage,note,noteID):
