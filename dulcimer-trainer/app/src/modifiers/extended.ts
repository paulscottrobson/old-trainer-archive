...
class MyPlayer extends MusicPlayer {
    ...
    private static NOTETOINDEX:any = {
        "c":0,"c#":1,"d":2,"d#":3,"e":4,"f":5,"f#":6,"g":7,"g#":8,"a":9,"a#":10,"b":11
    }

    getTuning() : number[] {
        var tuning:string = this.music.tuning.toLowerCase() ;
        if (tuning == "dad") { tuning = "d0,a0,d1"; }
        if (tuning == "daa") { tuning = "d0,a0,a0"; }
        var tuningList:number[] = [];
        for (var baseNote of tuning.toLowerCase().split(",")) {
    ...
    getStringVolume(str:number) : number {
        return (str == 2 ? 100 : 50);
    }
}
