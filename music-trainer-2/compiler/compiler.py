...
class Compiler:
	...
	def processAssignments(self,src,sourceFile):
		self.assignments = { "beats":"4","tempo":"120","syncopation":"50","translator":"paul robson","author":"","strings":"4" }
		...
		self.name = self.assignments["type"].lower()
		if self.name == "dulcimer":														# differet strings
			self.assignments["strings"] = "3"
			if "tuning" not in self.assignments:										# unspecified use DAd
				self.assignments["tuning"] = "d3,a3,d4"
	...
	def getStrings(self):
		return int(self.assignments["strings"])
	...
	def isLowAtTop(self):
		return self.name == "dulcimer"													# is lowest note at top of tabs like on a dulcimer ?

	def getLoHiStrings(self):
		strings = "gcea"
		if self.name == "mandolin":
			strings = "gdae"
		if self.name == "dulcimer":
			strings = "bcm"
		return strings
