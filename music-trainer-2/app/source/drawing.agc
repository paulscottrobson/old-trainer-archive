...
function DRAWFretboard()
	...
	for s = 1 to 4																						// Draw 4 strings (2 melody)
		...
		SetSpritePosition(SPR_STRING+s,0,s*50)
		SetSpriteVisible(SPR_STRING+s,0)
	next s
	...
	SetSpritePosition(SPR_BALL,ctl.barX-GetSpriteWidth(SPR_BALL)/2,-99)
endfunction

// ****************************************************************************************************************************************************************
//														Function to process strings position, visibility etc.
// ****************************************************************************************************************************************************************

function DRAWProcessStrings(song ref as Song)	
	idString = IMG_STRING
	if song.typeID = "m" then idString = IMG_MSTRING
	for s = 1 to song.strings
		SetSpritePosition(SPR_STRING+s,0,__DRAWGetStringY(s,song))
		SetSpriteVisible(SPR_STRING+s,1)
		SetSpriteImage(SPR_STRING+s,idString)
		if song.typeID = "d" and s = 3 then SetSpriteImage(SPR_STRING+s,IMG_MSTRING)
		s1 = song.strings - s + 1
		if song.typeID = "u" and s = 1 then s1 = 1
		SetSpriteSize(SPR_STRING+s,ctl.screenWidth,ctl.screenHeight*1/100+s1*2)
	next s
	for s = song.strings+1 to 4
		SetSpriteVisible(SPR_STRING+s,0)
	next s
endfunction
		...
		if song.bars[bar].isStrummed = 0													
			for s = 1 to song.strings																	// Note boxes
				...
				if song.bars[bar].notes[n].fret[s] <> 99
					...
					s$ = str(floor(mod(fret,50)))														// As a string, integer part
					if fret > 50 then s$ = s$ + "+"
					SetTextString(id,s$)	
					col = floor(fret)
...
function DRAWMoveBar(song ref as Song,bar as integer,x as integer)
	...
	for n = 1 to song.bars[bar].noteCount 												
		...
		if xc < ctl.barX then alpha = 255-(ctl.barX-xc) else alpha = 255
		if song.bars[bar].isStrummed = 0
			for s = 1 to song.strings																	// Note boxes
				yc = __DRAWGetStringY(s,song)
				id = baseID + n * 10 + s
				if song.bars[bar].notes[n].fret[s] <> 99
					...
					if xc < ctl.barX then SetSpriteColor(id,128,128,128,alpha)
...
function __DRAWGetStringY(s as integer,song ref as Song)
	y = ctl.fretY + ctl.fretHeight / 2																	// Work out centre
	if ctl.flipFretboard <> 0 then flip = -1 else flip = 1 												// Flip Fretboard
	if song.isFlipped <> 0 then flip = -flip 															// Further flipped
	c = song.strings 																					// Number of strings
	y = y - (s - (c+1)/2.0) * flip * ctl.fretHeight / (c+1)
endfunction y
