...
function CHCTLSetup(song ref as Song,cdNow ref as ChordView,cdNext ref as ChordView)
	...
	CHORDSetup(cdNow,20,130,320,120,600,"Now",song.strings)
	CHORDSetup(cdNext,420,130,320,120,700,"Next",song.strings)
	CHORDSetAlpha(cdNow,0,song.strings)
	CHORDSetAlpha(cdNext,0,song.strings)
...
function CHCTLUpdate(song ref as Song,position# as float,lastPosition# as float,cdNow ref as ChordView,cdNext ref as ChordView)
	...
	if __CHCTLUpdateStartTime <> 0
		...
		CHORDSetAlpha(cdNow,alpha,song.strings)
		CHORDSetAlpha(cdNext,alpha,song.strings)
...
function CHCTLSetChord(song ref as Song,n as integer,cdNow ref as ChordView,cdNext ref as ChordView)
	if song.chordCount = 0 then exitfunction
	CHORDSetAlpha(cdNow,255,song.strings)
	CHORDSetChord(cdNow,song.chordEvents[n],song.strings)
	CHORDSetAlpha(cdNext,255,song.strings)
	if n < song.chordCount
		CHORDSetChord(cdNext,song.chordEvents[n+1],song.strings)
	else
		CHORDSetChord(cdNext,song.chordEvents[n],song.strings)
	endif
endfunction
