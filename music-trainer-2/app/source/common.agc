...
#constant SND_METRONOME 	(199)																		// Metronome SFX 
...
type Song
	instrument as string 																			// Instrument full name
	typeID as string 																				// Instrument ID (M)andolin (U)kulele
	isFlipped as integer 																			// True if flipped.
	...
	tuning as string 																				// Dulcimer tuning
endtype
...
function COMSetup()
	ctl.screenWidth = 1280																			// Screen size
	ctl.screenHeight = 768
	ctl.barX = 160
	ctl.barWidth = 420
	ctl.fretY = 300																					// Fretboard position
	ctl.fretHeight = 360
	ctl.sineY = ctl.fretY+ctl.fretHeight*0/100														// Sine position
	...
	SetWindowTitle("The Music Trainer (AGKvsn) : build "+str(BUILD_NUMBER)+" ("+BUILD_DATE+")")		// Screen set up
	...
	SetErrorMode(2)
	SetPrintColor(255,128,0)
	SetPrintSize(24.0)
	img = CreateSprite(LoadImage(GFXDIR+"background.jpg"))											// Background image
