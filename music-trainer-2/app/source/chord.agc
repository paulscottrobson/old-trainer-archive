...
function CHORDSetup(cv ref as ChordView,x as integer,y as integer,width as integer,height as integer,id as integer,label as String,stringCount as integer)
	cv.x = x
	cv.y = y
	cv.width = width
	cv.height = height
	...
	for i = 1 to stringCount																			// Strings
		y1 = __CHORDY(cv,i,stringCount)
...
function CHORDDelete(cv ref as ChordView,stringCount as integer)
	DeleteSprite(cv.id)
	DeleteText(cv.id)
...
function CHORDSetChord(cv ref as ChordView,cev ref as ChordEvent,stringCount as integer)
	for s = 1 to stringCount
		...
		y = __CHORDY(cv,s,stringCount) - GetSpriteHeight(cv.id+s+40)/2
...
function __CHORDY(cv ref as ChordView,y as integer,stringCount as integer)
	y = cv.y + cv.height / 2 + (y - (stringCount+1)/2.0) * cv.height / stringCount
endfunction y
...
function CHORDSetAlpha(cv ref as ChordView,alpha as integer,stringCount as integer)
	SetSpriteColorAlpha(cv.id,alpha)
	SetTextColorAlpha(cv.id,alpha)
	for i = 1 to stringCount
		SetSpriteColorAlpha(cv.id+i,alpha)
		if i < stringCount then SetSpriteColorAlpha(cv.id+i+40,alpha)
	next i
	for i = 0 to CHORD_CHROM
