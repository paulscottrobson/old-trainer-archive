...
function __SONGAddNoteEvent(song ref as Song,bar as integer,millibars as integer,action$ as string)
	...
	song.bars[bar].notes[n].name$ = mid(action$,song.strings*2+2,9999)									// Name if any
...
function __SONGAssign(song ref as Song,assign$ as string,value$ as string)
	...
	select assign$
		case "type"
			ASSERT(value$ = "ukulele" or value$ = "ukelele" or value$ = "mandolin" or value$ = "dulcimer","Unknown instrument "+value$)
			song.instrument = value$
			song.typeID = left(value$,1)
			song.isFlipped = (value$ = "dulcimer")
		endcase
		case "beats"
		...
		case "tuning"
			song.tuning = value$
			ASSERT(CountStringTokens(value$,",") = 3,"Tuning must have three values")
		endcase
		case "tempo"
...
function __SONGClear(song ref as Song)
	song.instrument = "nonexistent" 																	// No instrument given
	song.typeID = "?"
