...
class HorizontalScrollRenderer extends Phaser.Group implements IRenderer {
    ...
    constructor(game:Phaser.Game,bar:IBar,music:IMusic,width:number,height:number) {
        super(game);
    ...
    private renderBar(): void {
        ...
        var c:number = (this.music.getVoices() == 3) ? 4 : this.music.getVoices();
    ...
    private drawNotesSeperate(): void {
        ...
        for (var strum of this.bar.strums) {
            ...
            for (var strn:number = 0;strn < this.music.getVoices();strn++) {
                if (strum.fretting[strn] != Strum.SILENT) {
                    ...
                    var s1:string = HorizontalScrollRenderer.NAMES[n];
                    // Put text therein.
    ...
    /**
     * Names of notes to be displayed (Seagull Merlin)
     * 
     * @private
     * @static
     * @type {string[]}
