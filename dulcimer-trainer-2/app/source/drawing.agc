...
function DRAWFretboard()
	...
	for s = 1 to 4																						// Draw 4 strings (2 melody)
		...
		y1 = __DRAWGetStringY(4-s)
		if s >= 3 then y1 = __DRAWGetStringY(1) - GetSpriteHeight(SPR_STRING+s)*((s-3)*2-1)
		SetSpritePosition(SPR_STRING+s,0,y1)
	next s
		...
		if song.bars[bar].isStrummed = 0 												
			for s = 1 to song.strings																	// Note boxes
				...
				if song.bars[bar].notes[n].fret[s] <> 99
					...
					s$ = GetStringToken(ctl.fretName$,",",fret+1)										// Convert to diatonic fret name
					SetTextString(id,s$)	
					col = floor(fret)
...
function __DRAWGetStringY(s as integer)
	...
	y = y - (s - 2) * flip * ctl.fretHeight * 30 / 100													// Adjust by fret
endfunction y
