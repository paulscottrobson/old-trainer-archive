...
type Constants																							// Control constants
	...
	fretName$ as string 																				// CS names of frets
	tuning$ as string 																					// Song tuning
endtype
...
type NoteEvent
	...
	fret as integer[3]																				// Chromatic fret position of note, 99 if not played.
...
type ChordEvent
	...
	fret as integer[3]																				// Chromatic fre position of note, 99 if not played.
endtype
...
function COMSetup()
	...
	ctl.fretName$ = "0,0+,1,1+,2,3,3+,4,4+,5,6,6+,7,7+,8,8+,9,10,10+,11,11+,12,13,13+,14"
	OpenToRead(1,"flipfretboard.txt")																// Check if flipped.
	a$ = ReadLine(1)
	CloseFile(1)	
	ctl.flipFretboard = asc(lower(a$)) = asc("y")
	
	SetWindowTitle("Dulcimer Trainer : build "+str(BUILD_NUMBER)+" ("+BUILD_DATE+")")				// Screen set up
